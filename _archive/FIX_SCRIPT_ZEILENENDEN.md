# 🔧 Script-Zeilenenden Fix

**Problem:** `cannot execute: required file not found`

**Ursache:** Windows-Zeilenenden (CRLF) statt Unix-Zeilenenden (LF)

---

## ✅ Lösung: Auf Server ausführen

**Auf Server (167.235.224.149):**

```bash
# 1. Zeilenenden konvertieren
sed -i 's/\r$//' check-all-frontend-routes.sh

# 2. Oder mit dos2unix (falls installiert)
dos2unix check-all-frontend-routes.sh

# 3. Ausführbar machen
chmod +x check-all-frontend-routes.sh

# 4. Testen
./check-all-frontend-routes.sh
```

---

## 🔄 Alternative: Script direkt auf Server erstellen

**Auf Server ausführen:**

```bash
cat > /opt/hd-app/The-Connection-Key/check-all-frontend-routes.sh << 'EOF'
#!/bin/bash
...
chmod +x /opt/hd-app/The-Connection-Key/check-all-frontend-routes.sh
```
