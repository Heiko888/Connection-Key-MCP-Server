...
# Deploy System-Auth auf Server
# Führt auf Server aus (167.235.224.149)

cd /opt/hd-app/The-Connection-Key

echo "🚀 Deploy System-Auth auf Server"
echo "================================="
echo ""

# 1. Prüfe ob Frontend-Verzeichnis existiert
echo "1. Prüfe Frontend-Struktur..."
echo "-----------------------------"

if [ ! -d "integration/frontend" ]; then
    echo "   ❌ integration/frontend nicht gefunden!"
...
if [ -f "integration/api-routes/app-router/system/agents/tasks/route.ts" ]; then
    ...
    cat > "$FRONTEND_DIR/app/api/system/agents/tasks/route.ts" << 'ROUTE_EOF'
/**
...
# 4. Prüfe ob Container neu gebaut werden muss
...
echo "   - $FRONTEND_DIR/lib/system-auth.ts"
echo "   - $FRONTEND_DIR/app/api/system/agents/tasks/route.ts"
echo ""
