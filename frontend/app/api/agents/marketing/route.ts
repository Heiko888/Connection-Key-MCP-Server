...
const MARKETING_AGENT_SYSTEM_PROMPT = `Du bist ein professioneller Marketing-Agent mit Expertise in:
- Social Media Marketing (Instagram, LinkedIn, Facebook, Twitter)
- Content-Marketing und Blog-Artikel
- Email-Marketing und Newsletter
- Werbetexte und Ad-Copy
- Marketing-Strategien und Kampagnenplanung
- Content-Kalender und Redaktionspläne
- SEO-optimierte Inhalte
- Branding und Markenentwicklung

Du hilfst dabei, ansprechende Marketing-Inhalte zu erstellen, die:
- Zielgruppen-orientiert sind
- Klare Call-to-Actions haben
- Professionell und überzeugend formuliert sind
...
- Erwähne die verschiedenen Subscription-Modelle wenn passend

Antworte immer auf Deutsch, sei präzise und hilfreich. Nutze die Brandbook-Informationen für alle Marketing-Content-Erstellungen.`;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Wenn taskId vorhanden, verwende diesen; sonst erstelle neue Task (Rückwärtskompatibilität)
    if (requestTaskId) {
      taskId = requestTaskId;
    } else {
      // Task erstellen (für Legacy-Support, aber sollte nicht passieren bei CoachAuth)
    ...
    try {
      const mcpResponse = await fetch(`${MCP_SERVER_URL}/agents/marketing`, {
        method: 'POST',
        headers: {
      ...
      if (mcpResponse.ok) {
        ...
        return NextResponse.json({
          success: true,
          response: response,
          message: 'Marketing agent response received',
          source: 'mcp-server',
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      {
        role: 'system',
        content: MARKETING_AGENT_SYSTEM_PROMPT
      },
    ...
    console.error('Error calling marketing agent:', error);
    ...
    return NextResponse.json(
      { 
        error: 'Failed to call marketing agent',
...
export async function GET(request: NextRequest) {
  return NextResponse.json({
    ...
    endpoint: `${MCP_SERVER_URL}/agents/marketing`,
    method: 'POST',
