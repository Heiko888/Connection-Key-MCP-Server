...
import { Send, User, Loader2, AlertCircle, Zap, Code, Workflow, Settings, FileCode, Terminal, GitBranch, Lightbulb } from 'lucide-react';
import { Box, Button, Chip, Typography, Card, CardContent, TextField } from '@mui/material';
...
interface AutomationAgentInterfaceProps {
  agentName: string;
...
const automationTemplates = [
  {
    category: 'Workflows & Prozesse',
    templates: [
      {
        id: 'workflow-automation',
        name: 'Workflow-Automatisierung',
        icon: Workflow,
        prompt: 'Erstelle einen Automatisierungsplan für folgenden Workflow:'
      },
      {
        id: 'deployment-automation',
        name: 'Deployment-Automatisierung',
        icon: Terminal,
        prompt: 'Erstelle einen Automatisierungsplan für das Deployment von:'
      },
      {
        id: 'data-processing',
        name: 'Datenverarbeitung',
        icon: FileCode,
        prompt: 'Erstelle einen Automatisierungsplan für die Datenverarbeitung von:'
      }
    ]
  },
  {
    category: 'Entwicklung & Code',
    templates: [
      {
        id: 'script-generation',
        name: 'Skript erstellen',
        icon: Code,
        prompt: 'Erstelle ein Skript für folgende Aufgabe:'
      },
      {
        id: 'ci-cd-pipeline',
        name: 'CI/CD Pipeline',
        icon: GitBranch,
        prompt: 'Erstelle eine CI/CD Pipeline-Konfiguration für:'
      }
    ]
  },
  {
    ...
    templates: [
      {
        ...
        icon: Settings,
        prompt: 'Entwickle einen Plan für die Integration folgender APIs/Services:'
      }
    ]
  }
];

export default function AutomationAgentInterface({
  agentName,
  agentDescription,
  ...
  const handleTemplateClick = (template: { prompt: string; name: string }) => {
    const prompt = `${template.prompt} [Bitte beschreibe deine Aufgabe/Anforderung]`;
    sendMessage(prompt);
  };
    ...
    >
      ...
      {showTemplates && messages.length <= 1 && (
        <Box sx={{ 
          ...
          {/* Nur erste Kategorie anzeigen, kompakt */}
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            {automationTemplates[0]?.templates.map((template) => (
              <Chip
                key={template.id}
                label={template.name}
            ...
            <Chip
              label="Mehr..."
              onClick={() => {
                // Zeige alle Kategorien in einem Dialog oder erweiterten Bereich
                const allTemplates = automationTemplates.slice(1).flatMap(cat => cat.templates);
                if (allTemplates.length > 0) {
                  handleTemplateClick(allTemplates[0]);
                }
              }}
              sx={{
