...
import { Send, User, Loader2, AlertCircle, Video, Instagram, Youtube, FileText, Calendar, TrendingUp, Lightbulb } from 'lucide-react';
...
interface SocialYouTubeAgentInterfaceProps {
  agentName: string;
...
const socialTemplates = [
  {
    ...
    description: 'Erstelle ein vollständiges YouTube-Video-Script',
    icon: Youtube,
    prompt: 'Erstelle ein vollständiges YouTube-Video-Script mit Einleitung, Hauptteil und Call-to-Action. Das Thema ist:'
  },
  {
    id: 'youtube-title',
    name: 'YouTube-Titel & Thumbnail',
    description: 'Erstelle ansprechende YouTube-Titel und Thumbnail-Beschreibungen',
    icon: Video,
    prompt: 'Erstelle 5 ansprechende YouTube-Titel und detaillierte Thumbnail-Beschreibungen. Das Video handelt von:'
  },
  {
    id: 'instagram-post',
    name: 'Instagram Post',
    description: 'Erstelle einen Instagram Post mit Caption und Hashtags',
    icon: Instagram,
    prompt: 'Erstelle einen Instagram Post mit ansprechender Caption, relevanten Hashtags und Call-to-Action. Das Thema ist:'
  },
  {
    id: 'instagram-story',
    name: 'Instagram Story',
    description: 'Erstelle Instagram Story-Ideen mit Texten',
    icon: Instagram,
    prompt: 'Erstelle 5 Instagram Story-Ideen mit Texten und interaktiven Elementen. Das Thema ist:'
  },
  {
    id: 'linkedin-post',
    name: 'LinkedIn Post',
    description: 'Erstelle einen professionellen LinkedIn Post',
    icon: TrendingUp,
    prompt: 'Erstelle einen professionellen LinkedIn Post mit Einleitung, Hauptteil und Call-to-Action. Das Thema ist:'
  },
  {
    id: 'content-calendar',
    name: 'Content-Kalender',
    description: 'Erstelle einen Social Media Content-Kalender',
    icon: Calendar,
    prompt: 'Erstelle einen 4-Wochen Content-Kalender für Social Media mit Themen, Post-Ideen, Hashtags und optimalen Posting-Zeiten. Der Fokus liegt auf:'
  },
  {
    id: 'video-strategy',
    name: 'Video-Strategie',
    description: 'Entwickle eine umfassende Video-Content-Strategie',
    icon: Video,
    prompt: 'Entwickle eine umfassende Video-Content-Strategie mit Themen, Formaten, Upload-Plan und Wachstums-Strategie. Für:'
  },
  {
    id: 'hashtag-research',
    name: 'Hashtag-Research',
    description: 'Finde relevante und trendige Hashtags',
    icon: TrendingUp,
    prompt: 'Führe eine Hashtag-Research durch und erstelle eine Liste mit relevanten, trendigen und Nischen-Hashtags. Für das Thema:'
  }
];

export default function SocialYouTubeAgentInterface({
  agentName,
  agentDescription,
  agentColor = 'from-red-500 to-pink-500',
  apiEndpoint,
  userId,
  className = ''
}: SocialYouTubeAgentInterfaceProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: `Hallo! Ich bin dein ${agentName}. ${agentDescription}\n\nIch kann dir helfen bei:\n• YouTube-Video Scripts erstellen\n• Instagram Posts & Stories entwickeln\n• LinkedIn Posts formulieren\n• Content-Kalender planen\n• Video-Strategien entwickeln\n• Hashtag-Research durchführen\n• Social Media Content optimieren\n\nWähle eine Vorlage oder stelle mir eine Frage!`,
  ...
  const handleTemplateClick = (template: { prompt: string; name: string }) => {
    const prompt = `${template.prompt} [Bitte beschreibe dein Thema/Video/Content]`;
    sendMessage(prompt);
  };
      ...
      {showTemplates && messages.length <= 1 && (
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            {socialTemplates.slice(0, 4).map((template) => (
              <Chip
                key={template.id}
                label={template.name}
            ...
            {socialTemplates.length > 4 && (
              <Chip
                label="Mehr..."
                onClick={() => {
                  const remainingTemplates = socialTemplates.slice(4);
          ...
          >
            {message.role === 'assistant' && (
              <Box sx={{
                ...
                <Video size={16} style={{ color: '#ffffff' }} />
              </Box>
            )}
