/**
 * Automation Agent API Route (App Router)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ============================================
    // SCHRITT 3: Automation Agent aufrufen
    // ============================================
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        return NextResponse.json({
          success: false,
          error: 'Automation Agent request timeout after 5 minutes',
          taskId
        }, { status: 504 });
      }
      throw fetchError;
...
export async function GET() {
  return NextResponse.json({
    ...
    example: {
      message: 'Erstelle einen n8n Workflow für automatische E-Mail-Benachrichtigungen'
    }
  });
}
