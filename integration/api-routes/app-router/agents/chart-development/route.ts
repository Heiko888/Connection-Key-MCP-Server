/**
 * Chart Development Agent API Route (App Router)
...
const AGENT_NAME = 'Chart Development Agent';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ============================================
    // SCHRITT 3: Chart Development Agent aufrufen
    // ============================================
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        return NextResponse.json({
          success: false,
          error: 'Chart Development Agent request timeout after 5 minutes',
          taskId
        }, { status: 504 });
      }
      throw fetchError;
...
export async function GET() {
  return NextResponse.json({
    success: true,
    message: 'Chart Development Agent API',
    endpoint: '/api/agents/chart-development',
    method: 'POST',
    ...
    example: {
      message: 'Analysiere einen Human Design Chart'
    }
  });
}
