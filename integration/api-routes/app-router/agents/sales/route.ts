/**
 * Sales Agent API Route (App Router)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ============================================
    // SCHRITT 3: Sales Agent aufrufen
    // ============================================
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        return NextResponse.json({
          success: false,
          error: 'Sales Agent request timeout after 5 minutes',
          taskId
        }, { status: 504 });
      }
      throw fetchError;
...
export async function GET() {
  return NextResponse.json({
    ...
    example: {
      message: 'Erstelle einen Verkaufstext für ein neues Produkt'
    }
  });
}
