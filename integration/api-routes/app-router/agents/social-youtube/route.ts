/**
 * Social-YouTube Agent API Route (App Router)
...
const AGENT_NAME = 'Social-YouTube Agent';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ============================================
    // SCHRITT 3: Social-YouTube Agent aufrufen
    // ============================================
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        return NextResponse.json({
          success: false,
          error: 'Social-YouTube Agent request timeout after 5 minutes',
          taskId
        }, { status: 504 });
      }
      throw fetchError;
...
export async function GET() {
  return NextResponse.json({
    ...
    endpoint: '/api/agents/social-youtube',
    method: 'POST',
    description: 'YouTube-Video-Skripte, Reels, Posts, Captions, Thumbnail-Ideen, Social-Media-Content',
    requiredFields: {
    ...
    example: {
      message: 'Erstelle ein YouTube-Video-Skript für ein Tutorial'
    }
  });
}
