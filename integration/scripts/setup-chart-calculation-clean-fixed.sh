#!/bin/bash
# Saubere Chart-Berechnungs-Implementierung (KORRIGIERT)
...
if grep -q "chart-calculation" "$SERVER_FILE"; then
    ...
    # Erstelle temporäre Datei mit Erweiterungen
    TEMP_FILE=$(mktemp)
    
    # Kopiere server.js in temporäre Datei
    cp "$SERVER_FILE" "$TEMP_FILE"
    
    # Füge require für chart-calculation hinzu (nach require('dotenv'))
    if grep -q "require('dotenv')" "$TEMP_FILE"; then
        sed -i "/require('dotenv')/a\\
const chartCalculationService = require('./chart-calculation');" "$TEMP_FILE"
    else
        # Falls dotenv nicht gefunden, füge nach express hinzu
        sed -i "/require('express')/a\\
const chartCalculationService = require('./chart-calculation');" "$TEMP_FILE"
    fi
...
ENDPOINTS_END
    ...
    # Ersetze server.js mit erweiterter Version
