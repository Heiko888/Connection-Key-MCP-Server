 ...
 * - BullMQ Worker für reading-queue (V3) und reading-queue-v4 (V4)
 * - loadKnowledge() und loadTemplates() als inline Funktionen
 * - Verwendet gemeinsame V4 Supabase Datenbank für beide Queues
 */
...
// ======================================================
// loadKnowledge() - inline (aus production/server.js)
// ======================================================
...
// ======================================================
// loadTemplates() - inline (aus production/server.js)
// ======================================================
...
// ======================================================
// Supabase Client (V4 - wird für BEIDE Queues verwendet)
// ======================================================
...
console.log("✅ Supabase (V4):", process.env.V4_SUPABASE_URL?.substring(0, 40) + "...");
...
// ======================================================
// generateReading() - Simplified Version
// ======================================================
async function generateReading({ agentId, template, userData }) {
  console.log("🤖 Generiere Reading:", { agentId, template });
...
// Nutzt V4 Supabase, aber coach_readings Tabelle
// ======================================================
const workerV3 = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // Update coach_readings (in V4 Supabase!)
...
// Nutzt readings + reading_jobs Tabellen
// ======================================================
const workerV4 = new Worker(
  ...
  async (job) => {
    ...
    try {
      // 1. Reading aus readings Tabelle holen
      ...
      // 2. Job Status auf "processing" setzen (falls reading_jobs existiert)
      ...
      // Ignoriere Fehler falls reading_jobs noch nicht existiert
      if (jobUpdateError) {
...
const connectionWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // 2. Job Status auf "processing" setzen
      await supabase
        .from("reading_jobs")
        .update({
          status: "processing",
      ...
      // 3. Charts für beide Personen berechnen (placeholder - wird später mit Chart-Truth-Service ersetzt)
      const personAChart = {
        type: "Generator",
        gates: [1, 2, 3],
        centers: ["Sacral", "Throat"],
        note: "Placeholder - Chart-Truth-Service Integration folgt"
      };
      ...
      // 4. Connection Dynamics analysieren
      const dynamics = analyzeConnectionDynamics(personAChart, personBChart);

      // 5. Update connection_readings mit Charts und Dynamics
      await supabase
        .schema("public")
...
const pentaWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // 3. Charts für alle Mitglieder berechnen (placeholder)
      const memberCharts = members.map((member, index) => ({
        name: member.name,
        chart: {
          type: index % 5 === 0 ? "Generator" : index % 5 === 1 ? "Manifestor" : index % 5 === 2 ? "Projector" : index % 5 === 3 ? "Reflector" : "Generator",
          gates: [1 + index, 2 + index, 3 + index],
          centers: ["Sacral", "Throat"],
          note: "Placeholder - Chart-Truth-Service Integration folgt"
        }
      }));

      // 4. Penta/Gruppen-Dynamik analysieren
      const groupDynamics = analyzePentaDynamics(memberCharts);

      // 5. Penta Chart berechnen (Composite Chart aller Mitglieder)
      const pentaChart = calculatePentaChart(memberCharts);

      // 6. Update penta_readings mit Charts und Dynamics
      await supabase
        .schema("public")
        .from("penta_readings")
        .update({
...
const multiAgentWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // 3. Alle Agents parallel ausführen
      ...
      // 5. Synthesis erstellen (wenn enabled)
      ...
      if (synthesis) {
        ...
        synthesisResult = await generateMultiAgentSynthesis(
          agentResults,
          birthData,
          knowledge
        );

        // Extract insights aus allen Readings
        combinedInsights = agentResults.flatMap(r => r.insights);

        // Cross-References finden (wo Agents ähnliche Punkte ansprechen)
        crossReferences = findCrossReferences(agentResults);

        console.log("✅ [Multi-Agent] Synthesis fertig");
      }

      // 6. Finales Reading zusammensetzen
      let finalContent = "";
      ...
      // 7. Update multi_agent_readings mit Results
...
// ======================================================
// Job Polling System (für Connection, Penta, Multi-Agent)
// Holt pending Jobs aus reading_jobs Tabelle
// ======================================================
async function pollForJobs() {
  try {
    // Hole alle pending Jobs (OHNE reading_id - existiert nicht in v4!)
    ...
    for (const job of pendingJobs) {
      try {
        // V4: Job enthält reading_type direkt, kein reading_id!
        let reading = null;
        let readingType = job.reading_type;

        // V4: Erstelle virtuelles Reading aus Job-Daten
        reading = {
          id: job.id,
        ...
        // Route basierend auf reading_type
...
// Human Design Job verarbeiten (V4 ohne reading_id)
async function processHumanDesignJob(job, reading) {
  console.log(`🔄 [Human Design] Verarbeite Job ${job.id}`);

  const { birthdate, birthtime, birthplace, name } = reading.client_data || {};

  // Job Status auf processing
  await supabase
    .from("reading_jobs")
...
async function processConnectionJob(job, reading) {
  ...
  // Charts berechnen (placeholder)
  const personAChart = { type: "Generator", gates: [1, 2, 3] };
  const personBChart = { type: "Manifestor", gates: [8, 14, 60] };
  ...
  const content = await generateReading({
    ...
    userData: { personA, personB, dynamics, connectionQuestion }
  });
...
async function processPentaJob(job, reading) {
  ...
  // Charts für alle Mitglieder (placeholder)
  const memberCharts = members.map((m, i) => ({
    name: m.name,
    chart: { type: ["Generator", "Manifestor", "Projector"][i % 3], gates: [1+i, 2+i] }
  }));
  ...
  const content = await generateReading({
    agentId: 'penta',
    template: 'relationship',
    userData: { groupName, members, groupDynamics, pentaChart }
  });
...
// Polling starten (alle 10 Sekunden)
...
Stil: Klar, präzise, nicht esoterisch.`;
  ...
  const completion = await openai.chat.completions.create({
    model: "gpt-4o",
    messages: [
      { role: "system", content: "Du bist ein erfahrener Meta-Analyst für Human Design Readings." },
...
// Helper: Extract Insights aus Reading
function extractInsights(readingText, agentId) {
  // Simple Implementation: Extrahiere Sätze als Insights
  const sentences = readingText.split(/[.!?]+/).filter(s => s.trim().length > 20);
  return sentences.slice(0, 5).map(s => s.trim());
}

// Helper: Find Cross-References
...
// ======================================================
// Helper: Analyze Penta/Group Dynamics
// ======================================================
function analyzePentaDynamics(memberCharts) {
  ...
  // Rollen basierend auf Types
  const typeCounts = {};
  memberCharts.forEach((member, index) => {
  ...
  // Energy Flow (welche Types dominieren)
  ...
  // Collective Strengths (alle einzigartigen Gates)
  const allGates = new Set();
  memberCharts.forEach(member => {
    member.chart.gates.forEach(gate => allGates.add(gate));
  });
  dynamics.collective_strengths = Array.from(allGates);

  // Team Challenges (was fehlt)
...
function calculatePentaChart(memberCharts) {
  // Penta Chart = Composite aller individuellen Charts
  const allGates = [];
  const allCenters = new Set();

  memberCharts.forEach(member => {
    allGates.push(...member.chart.gates);
    member.chart.centers.forEach(center => allCenters.add(center));
  });

  return {
    composite_gates: [...new Set(allGates)],
...
// ======================================================
// Helper: Analyze Connection Dynamics
// ======================================================
function analyzeConnectionDynamics(chartA, chartB) {
  ...
  const gatesA = chartA.gates || [];
  const gatesB = chartB.gates || [];

  // Finde Gates, die zusammen Channels bilden
...
app.get("/health", async (_, res) => {
  try {
    // Prüfe Supabase
    const { error: dbError } = await supabase.schema("public").from("readings").select("id").limit(1);
...
app.listen(4000, () => {
  console.log("🚀 Combined Reading Worker läuft auf Port 4000");
  console.log("   - V3 Queue: reading-queue (coach_readings in V4 DB)");
  console.log("   - V4 Queue: reading-queue-v4 (readings + reading_jobs)");
  console.log("   - Connection Queue: reading-queue-v4-connection (2 Personen)");
  console.log("   - Penta Queue: reading-queue-v4-penta (3-7 Personen)");
  console.log("   - Multi-Agent Queue: reading-queue-v4-multi-agent (parallel agents)");
